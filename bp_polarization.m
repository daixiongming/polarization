...
numLabels = 256;        % 256个像素
numLabels_new=90;       %入射角的范围,每一个入射角参数作为已知数来参与迭代。

numIter = 5;        % number of BP iterations at each scale
numLevels = 5;      % number of scales


img = importdata('penguin.png'); %just for test
%img=importdata('cup.jpg');
img = img.cdata;
figure(1);
imshow(img);
...
dataCost_new=zeros(height,width,numLabels_new);

for i=1:numLabels_new
    gg = min((img-(i-1)).^2,data_k);
    dataCost_new(:,:,i) = lambda*gg;
end
% 
% for i=1:numLabels
%     gg = min((img-(i-1)).^2,data_k);
